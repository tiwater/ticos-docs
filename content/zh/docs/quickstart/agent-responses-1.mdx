...
![添加终端类型](/images/quickstart-agent-responses/image2.png)

保存后返回列表。点击“编辑型号”，进入下级界面，在浏览器地址栏最后的数字是型号 ID，记录下来后面访问接口备用。

![编辑详细信息](/images/quickstart-agent-responses/image3.png)
...
4. 在智能体“指令”中输入希望的业务逻辑，例如：数一数图片中有几个人，用 json 格式返回 ```{"count": n}``` 。
...
![获取 API-Key](/images/quickstart-agent-responses/image8.png)
